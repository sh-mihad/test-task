...
export default function TaskList({onDeletTask ,tasks , onEditTask ,onTaskFavourite}) {
  return (
    ...
    {tasks.map((task) => (
        ...
        <tr key={task.id} className="border-b border-[#2E3443] [&>td]:align-baseline [&>td]:px-4 [&>td]:py-2">
            <td>
            <button onClick={()=>onTaskFavourite(task.id)}>
        {task.isFavourite ? <FaStar className="text-yellow-500" /> : <FaStar className="text-gray-500" />}
            </button>
            </td>
            <td>{task.title}</td>
            <td>
            ...
            <td>
                <div className="flex items-center justify-center space-x-3">
                    <button onClick={()=>onDeletTask(task.id)} className="text-red-500">Delete</button>
